...
export interface ExtendedResourcesConfig extends ResourcesConfig {
  region: string;
  domainName: string;
}
...
export async function configureAmplify(): Promise<ExtendedResourcesConfig> {
  if (!awsExports) {
    try {
      ...
      console.debug("Fetched AWS exports:", awsExports);
    } catch (error) {
