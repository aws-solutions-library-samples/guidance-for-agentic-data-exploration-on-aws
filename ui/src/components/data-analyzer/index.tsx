...
import { DataAnalyzerDetailComponent } from './detail';
...
const fallbackRender = ({error, resetErrorBoundary}:FallbackProps) => { 
  console.error('error in Data Analyzer', error);   
  return (
...
export function DataAnalyzerTable(props:{ items: DataAnalyzerItem[], loading: boolean, onRefresh?: () => void }) {
  ...
  const columnDefinitions = [
    ...
    {
      id: 'file_name',
      header: 'S3 Prefix Analyzed',
      cell: (item: DataAnalyzerItem) => <Link><div onClick={()=>navigate(item.id)}>{item.file_name}</div></Link>,
    ...
    },
    {
      id: 'file_count',
      header: 'Files',
      cell: (item: DataAnalyzerItem) => item.file_count,
      sortingField: 'file_count',
      minWidth: 90,
      width: 90,
    },
  ];  
  ...
  const { items: collectionItems, filteredItemsCount, collectionProps, propertyFilterProps, paginationProps } = 
    useCollection(props.items, {
      filtering: {
        empty: 'No data analyzer items match the filters',
  ...
  return (
    <>
      <Table
        ...
        empty={<EmptyState title="No items found" subtitle="No data analyzer items match the criteria" />}
...
export default function DataAnalyzerScreen() {
  const [flashbarItems, setFlashbarItems] = useState<FlashbarProps.MessageDefinition[]>([])

  const { data, dataDetail, isLoading, isLoadingDetail, screenName, selectedItem, refetch } 
    = useScreenQuery<DataAnalyzerItem>();

  const breadcrumbs = [
    { text: 'Data Analyzer', href: `#/${screenName}` },
  ...
  return (
    ...
    <AppLayout
        ...
        content={
          (selectedItem ?
            ( dataDetail ? 
              (<DataAnalyzerDetailComponent item={dataDetail!} />) : 
                (isLoadingDetail ?
          ...
          <ErrorBoundary fallbackRender={fallbackRender}>
            <DataAnalyzerTable loading={isLoading} items={data||[]} onRefresh={refetch}>
            </DataAnalyzerTable>
          </ErrorBoundary>
          )
        )}
        tools={
          <HelpPanel data-testid="help-panel"
            header={<Header variant="h2">Data Analyzer Help</Header>}
          >
            <div>
              <p>The Data Analyzer screen shows the history of data analysis operations.</p>
