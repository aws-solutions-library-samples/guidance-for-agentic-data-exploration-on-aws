describe('Data Loader Page', () => {
  beforeEach(() => {
    cy.authenticate('/panoptic/e2e-testing')
    cy.visit('/')
    // Navigate to data loader page using side nav
    cy.get('a[href*="data-loader"]').first().click();
  });

  it('should display the Data Loader screen with table', () => {
    // Verify main header exists
    cy.contains('Data Loader Status').should('exist');
    
    // Verify table exists and is visible
    cy.get('table').should('exist').and('be.visible');
    
    // Verify key table columns are present
    cy.contains('th', 'Source Path').should('exist');
    cy.contains('th', 'Start Time').should('exist');
    cy.contains('th', 'Records').should('exist');
    cy.contains('th', 'Status').should('exist');
    cy.contains('th', 'Load Time').should('exist');
  });
  ...
  it('should display help panel with guidance', () => {
    ...
    cy.get('[data-testid="help-panel"]')
      .should('exist')
      .within(() => {
        cy.contains('Data Loader Help').should('exist');
        cy.contains('The Data Loader screen shows the status of bulk data loading operations').should('exist');
      });
  });
